...
class optimize_mgh:
    ...
    def outer_minimizer(self, lamb):
        prev_loglike = 0
        if self.mu == None and self.gamma == None:
        ...
        while True:
            ...
            #    print(e)
            #    lamb = lamb - 3 * np.random.uniform(0, 1)
            #    errorhit = True
            if lamb == -1.0:
...
def outer_minimizer(
    ...
    loglike_t = lambda k, mu, nu: k.apply(
        lambda row: np.log(
            multivariate_t_distribution(
                row[ret_col], mu, row["cov_t"], nu, len(ret_col)
            )
        ),
        axis=1,
    ).mean()
    ...
    prev_loglike = (
        op.loglike(chi) if dist_type == "skew" else loglike_t(k, mu, chi)
    )
    print(f"Begining Log likelihood ={prev_loglike}")
    ...
    while True:
        ...
        print(f"iter = {it}")
        # try:
        normlog = op.multskewt(lamb, l=l, useRMT=useRMT, dist_type=dist_type)
        print(f"normlog={normlog}")
        # except ValueError as e:
        #    print(e)
        #    lamb = lamb - 3 * np.random.uniform(0, 1)
        ...
        if dist_type == "skew":
            curr_loglike = op.loglike(chi)
        else:
            curr_loglike = loglike_t(k, mu, chi + 2)
        print(f"Current Loglikelihood={curr_loglike}")
        print(f"Current value of lamb={lamb}")
...
op = optimize_mgh(ktrain, useRMT=False, useREig=False, dist_type="t")
