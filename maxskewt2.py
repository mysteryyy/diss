...
class optimize:
    ...
    def loglike_gig2(self, chi, psi=1e-10):
        length = len(self.k)
        nu = chi
        g = self.k["gt"]
        lnt = self.k["lngt"]
        ginv = self.k["ginvt"]
        term1 = -nu * np.log(nu - 1) / 2
    ...
    def loglike_gig1(self, chi, psi=1e-10):
        v = chi
        length = len(self.k)
        lnt = np.log(self.k["gt"]).sum() / length
        ginv = self.k["ginvt"].sum() / length
        ginvlog = np.log(self.k["ginvt"]).sum() / length
        logval = (
            digamm((v + self.d) / 2)
            - digamm(v / 2)
            + np.log(v / (v + self.d))
...
def outer_minimizer(chi, k):
    prev_loglike = 0
    mu = np.matrix(k[ret_cols].mean()).T
    gamma = np.matrix(k[ret_cols].skew()).T
    op = optimize(mu, gamma, k)

    def inner_minimizer(params):
        chi = params[0]
        giglog = op.loglike_gig2(chi)
...
res, chi, curr_loglike = outer_minimizer(15.7, ktrain)
print(chi)
