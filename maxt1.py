...
from gig import lnw, log_gamma, loglike_ig, moment_bessel, moment_gamma
...
class optimize:
    ...
    def gigparams(self, lamb, chi, psi=0):
        ...
        k["gt"] = k.apply(
            lambda row: moment_gamma(-lamb, row["chistar_t"], 1),
            axis=1,
        )
        k["ginvt"] = k.apply(
            lambda row: moment_gamma(-lamb, row["chistar_t"], -1),
    ...
    def loglike_gig(self, chi, psi=1e-10):
        ...
        lnt = self.k["lngt"]
        ginv = self.k["ginvt"]
        iglog = loglike_ig(lnt, ginv, -lamb, chi)
        return iglog.sum() / len(self.k)
